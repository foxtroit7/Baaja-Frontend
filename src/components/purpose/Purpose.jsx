...
const Purpose = () => {
  ...
  const fetchPurposes = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/purpose');
  ...
  const handleSave = async () => {
    try {
      if (isEditMode) {
        await axios.put(`http://localhost:5000/api/purpose/${selectedPurpose._id}`, formData);
        toast.success('Purpose updated successfully');
      } else {
        await axios.post('http://localhost:5000/api/purpose', formData);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/purpose/${selectedPurpose._id}`);
